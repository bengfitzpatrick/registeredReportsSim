...
load("D:\\zdrive\\tempest\\projects\\integrity\\registeredReports\\nh50\\pH0p8dp5r50_50k_r_10.Rdata")
...
#
# create time series plot objects
#
effPlot <- ggplot(data=tsDF,aes(x=x,y=eff))+geom_line(aes(color = simCondition),linewidth=2) + 
 ...
 nhPlot <- ggplot(data=tsDF,aes(x=x,y=nh))+geom_line(aes(color = simCondition),linewidth=2) + 
  scale_color_manual(values = c("black","blue","red")) + ylim(c(0,50)) + #xlim(c(1.5,3.5)) + 
...
#
#  create grid of four time series plots
#
...
scale_color_manual(values=c("#000000", "#090de0","#de0a02" ))+scale_fill_manual(values=c("#404040", "#090de0", "#de0a02"))+
xlim(c(0,50)) + xlab("Number of Hypotheses") + scale_x_continuous(breaks=seq(0,50,by=5))
print(p3)
dev.new(width=12,height=8)
p4<-ggplot(histDF, aes(x=value, color=SimCondition, fill=SimCondition)) + geom_histogram(position = "identity", alpha = 0.5, binwidth = 10, boundary = 0) +
scale_color_manual(values=c("#000000", "#090de0","#de0a02" ))+scale_fill_manual(values=c("#404040", "#090de0", "#de0a02"))+
xlim(c(0,500)) + xlab("Number of Hypotheses") + scale_x_continuous(breaks=seq(0,500,by=10))
print(p4)
