...
nFiles <- 12 
fName  <- "pH0p8dp5r10_50ka"

gTitle <- "Pr[H0] = 0.8, effects below 0.5 considered null, max number of hypotheses = 10"
...
myLinePlot <- ggplot(data=newDF,aes(x=x,y=y))+geom_line(aes(color = z, linetype = z),linewidth=2) + 
  scale_color_manual(values = c("darkred","steelblue","black")) + ylim(c(0,1)) + xlim(c(1.0,2.5)) + 
